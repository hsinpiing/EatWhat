const HistoryPage = {
  ...
  renderCalendar(monthData) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      html += `<div class="day-cell py-1 rounded-lg cursor-pointer ${isSelected ? 'bg-primary/20 ring-1 ring-primary' : hasData ? 'hover:bg-gray-800' : ''}"
                    data-date="${dateStr}">
  ...
  showDayDetail(date, records) {
    ...
    list.innerHTML = records.map(r => {
      ...
      return `<div class="flex items-center gap-2 py-1 border-b border-gray-700 last:border-0">
        <span class="text-lg">${r.emoji || '🍽️'}</span>
        <div class="flex-1">
          <p class="text-sm font-medium">${r.name}</p>
          ${time ? `<p class="text-xs text-gray-500">${time}</p>` : ''}
        </div>
      </div>`;
    }).join('');
